...
class Window(ThemedTk):
    ...
    def radio_button_click(self, selected_sitename: str):
        ...
        # currentEdit
        dataframe: DataFrame = datasource.get_plot_data(
            sitename=selected_sitename)
        axes = dataframe.plot()
        ...
        self.canvas.get_tk_widget().pack(
            side=tk.TOP, fill=tk.BOTH, expand=True, pady=(20, 10))

    def item_selected(self, event):
        for selected_item in self.tree.selection():
            record = self.tree.item(selected_item)
            dialog = view.MyCustomDialog(
                parent=self, title=f'{record["values"][1]}-{record["values"][2]}', record=record['values'])
